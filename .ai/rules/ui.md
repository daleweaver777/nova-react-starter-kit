...
Do not "fix" it by dropping `md:text-sm`: `text-sm` everywhere makes iOS zoom on every field focus, and `text-base` everywhere leaves inputs 2px larger than the labels and buttons beside them.

If it needs to be uniform across window widths, width is the wrong axis. Tailwind 4.3 supports `text-sm pointer-coarse:text-base`, which keeps resized desktop windows at 14px while coarse-pointer devices retain 16px.
...
Menus are the opposite case: `min-w-*` on a dropdown popup is a _minimum_ that should densify with the rest, so the spacing scale is right there. `DropdownMenuSubContent` uses `min-w-24`, which is exactly stock's 96px at default density.
